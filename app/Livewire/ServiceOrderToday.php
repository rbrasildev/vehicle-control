...
class ServiceOrderToday extends Component
{
    ...
    public function loadOpen()
    {
        ...
        $query = DB::connection($this->currentConnection)
            ...
            ->whereIn('atendimento_os.status', [0, 2, 3, 4]);
        ...
        $result = DB::connection($this->currentConnection)->table('admcore_clientecontrato')
            ->join('admcore_pop', 'admcore_pop.id', '=', 'admcore_clientecontrato.pop_id')
            ->join('atendimento_ocorrencia', 'atendimento_ocorrencia.clientecontrato_id', '=', 'admcore_clientecontrato.id')
            ->join('atendimento_os', 'atendimento_os.ocorrencia_id', '=', 'atendimento_ocorrencia.id')
            ->select(
                DB::raw('COUNT(*) AS total'),
                DB::raw("CASE
                    WHEN atendimento_os.status = 0 THEN 'Abertas'
                    WHEN atendimento_os.status = 1 THEN 'Finalizadas'
                    WHEN atendimento_os.status = 2 THEN 'Execução'
                    WHEN atendimento_os.status = 3 THEN 'Pendentes'
                    ELSE 'Outros'
                END AS status_descricao,
                atendimento_os.status
                ")
            )
            ->whereDate('atendimento_os.data_agendamento', DB::raw('CURRENT_DATE'));
        if (!empty($this->currentPop)) {
            $result->where('admcore_pop.id', $this->currentPop);
        }
        $result->groupBy('atendimento_os.status');
        $this->statusCounts = $result->get();
    ...
    {
        return view('livewire.service-order-today', ['clientes' => $this->loadOpen(), 'pops' => $this->getPop()]);
    }
}
