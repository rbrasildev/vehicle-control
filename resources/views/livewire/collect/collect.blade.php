    <div>
        <div class="flex gap-2 mb-2">
            ...
            <x-mary-choices-offline class="w-64" wire:model.live="currentPop" :options="$pops"
                placeholder="Selecione um pop" single />
        </div>
        <div class="flex gap-2 mb-2 justify-between items-center flex-wrap">
            <div class="flex gap-2 flex-wrap">
                @foreach ($models as $model)
                    <x-mary-button spinner="sort" label="{{ $model->onutype }}"
                        wire:click="modelSort('{{ $model->onutype }}')"
                        class="bedge-secondary btn btn-ghost dark:btn-neutral" spinner="modelSort('{{ $model->onutype }}')"
                        badge="{{ $model->total }}" badge-classes="badge-success" responsive />
                @endforeach
                ...
                <tbody>
                    @forelse ($collectData as  $collect)
                        <tr wire:key="cliente-{{ $collect->cliente_id }}">
                            ...
                            <td>{{ \Carbon\Carbon::parse($collect->data_alteracao)->diffForHumans() }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td>
